...
app.post('/sign-up', async (req, res) => {
  ...
  const existingUser = await User.findOne({ idNumber });
  if (existingUser) {
    return res.send('User with this ID already exists. Please login.');
  }
...
app.get('/dashboard', async (req, res) => {
  try {
    ...
    /* YEAR + MONTH (from query or today) */
    const queryMonth = parseInt(req.query.month);
    const queryYear = parseInt(req.query.year);

    const month = isNaN(queryMonth) ? moment().month() : queryMonth;
    const year = isNaN(queryYear) ? moment().year() : queryYear;

    const today = moment().format('YYYY-MM-DD');
    const daysInMonth = moment({ year, month }).daysInMonth();

    /* FIRST DAY OF MONTH (🔥 moved from EJS) */
    const firstDay1 = moment({ year, month, day: 1 }).day();
    ...
    let calendar = [];
    for (let d = 1; d <= daysInMonth; d++) {
      const date = moment({ year, month, day: d }).format('YYYY-MM-DD');
      calendar.push({
        date,
        isToday: date === today,
        isPast: moment(date).isBefore(today),
